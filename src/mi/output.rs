...
use nom::character::complete::{char, digit0, line_ending, multispace1};
...
pub fn process_output<T: Read>(output: T) -> Result<(), String> {
...
fn token(input: &str) -> IResult<&str, Token> {
    map(digit0, |values: &str| values.parse::<u64>().unwrap()).parse(input)
}
...
mod test {
    ...
    fn test_result_record() {
        let _ = Output::parse(
            "^done,bkpt={number=\"1\",type=\"breakpoint\",disp=\"keep\",enabled=\"y\",addr=\"0x0000000000018fdf\",func=\"test_app::main::{async_block#0}\",file=\"src/bin/test_app.rs\",fullname=\"/home/lz/workspace/mcp_server_gdb/src/bin/test_app.rs\",line=\"5\",thread-groups=[\"i1\"],times=\"0\",original-location=\"test_app.rs:5\"}",
        );
    }
}
